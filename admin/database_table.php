...
// returns nothing
//
function PackMasterWeb_table_one_form($editing, $heading)
{
	...
	if ($editing)
	{
		list($result, $error) = PackMasterWeb_get_table_one_result($param_table_one_key);
		if ($error)
		{
    ...
    <table border='1' cellpadding='0' cellspacing='0' width='100%'>\n
        ...
        <td class='bg2'>\n
			...
			$field_list = PackMasterWeb_get_table_one_fields();
            foreach ($field_list as $field => $prompt)
            {
				$pname = "param_" . $field;
				if ($editing OR ($field != 'table_one_key'))
				{
		            print "
					<tr nowrap='nowrap'>\n
					<td class ='head'>" . $prompt . "</td>\n
	                <td class='even'>\n";
					if ($field == 'table_one_key')
					{
						print $values[$field];
...
// returns nothing
//
function PackMasterWeb_table_one_del_form()
{
	global $param_table_one_key;
...
{
	global $xoopsDB;
	$bci = PackMasterWeb_get_table_one_fields();
	
	$sql =	"select " . PackMasterWeb_to_string($bci) . " from " . $xoopsDB->prefix("PackMasterWeb_table_one");

	if ($table_one_key != null)
	{
		...
		{
			$error = array('msg' => _AM_PMW_ERR_table_one_BAD_KEY, 'data' => $sql);
...
// returns nothing
//
function PackMasterWeb_table_one_main_select()
{
	$method = "POST";// POST, except when debugging
	global $xoopsDB;
	list($result, $error) = PackMasterWeb_get_table_one_result();
	if ($error)
	{
		printf(_AM_PMW_FMT_ERROR, $error['msg'], $error['data']);
		return;
	...
	if (!$are_there_any)
		print "<tr><td class = 'even' colspan = '" . (count($field_list)+1) . "'>". _AM_PMW_ERR_TABLE_ONE_NONE . "</td></tr>";
    print "</table>\n";
    print "</td></tr>\n";
	print "</table>\n";
}
...
//
function PackMasterWeb_table_one_main_select_post()
{
	xoops_cp_header();
    $p_title = _AM_PMW_TABLE_ONE_TITLE;
    print "<h4 style='text-align:left;'>$p_title</h4>";
...
//
function PackMasterWeb_table_one_delete()
{
	global $xoopsDB;
	global $param_table_one_key;
	
	$sql =	"delete from " .
		$xoopsDB->prefix("PackMasterWeb_table_one")
		. " where table_one_key = " 
		. "'" . mysql_escape_string($param_table_one_key) . "'";
	...
	if (!$xoopsDB->query($sql))
	{
		$error = $xoopsDB->error();
		xoops_cp_header();
		PackMasterWeb_show_sql_error(_AM_PMW_ERR_DELETE_FAILED, $error, $sql);
		xoops_cp_footer();
	}
	else 
	{
...
function PackMasterWeb_table_one_update()
{
	global $xoopsDB;
	$field_list = PackMasterWeb_get_table_one_fields();
	foreach ($field_list as $field => $prompt)
	...
	$sql =	"REPLACE INTO  " 
		. $xoopsDB->prefix("PackMasterWeb_table_one")
		. " (" . PackMasterWeb_to_string($field_list) . ") VALUES (";
	...
	if (!$xoopsDB->query($sql))
	{
		$error = $xoopsDB->error();
		xoops_cp_header();
		PackMasterWeb_show_sql_error(_AM_PMW_ERR_REPLACE_FAILED, $error, $sql);
		xoops_cp_footer();
	} else 
	{
		redirect_header("database_table.php",1,_AM_PMW_OK_DB);
