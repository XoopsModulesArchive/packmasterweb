...
function PackMasterWeb_table_one_edit_form(): void
{
    PackMasterWeb_table_one_form(true, _AM_PMW_LABEL_TABLE_ONE_EDIT);
    PackMasterWeb_table_one_del_form();
}

function PackMasterWeb_table_one_add_form(): void
{
    PackMasterWeb_table_one_form(false, _AM_PMW_LABEL_TABLE_ONE_ADD);
}
...
// returns nothing
function PackMasterWeb_table_one_form($editing, $heading): void
{
    global $param_table_one_key;
    global $xoopsDB;

    if ($editing) {
        [$result, $error] = PackMasterWeb_get_table_one_result($param_table_one_key);
        if ($error) {
            printf(_AM_PMW_FMT_ERROR, $error['msg'], $error['data']);
    ...
    $field_list = Utility::getTableOneField();
    ...
    print "<input type='hidden' name='op' value='edit_post'>\n
                ...
                <input type='submit' value='" . _AM_PMW_BUT_SAVE . "'>\n
                </td></tr>\n
            </table>\n
        </td></tr>\n
    </table>\n
...
// returns nothing
function PackMasterWeb_table_one_del_form(): void
{
    global $param_table_one_key;
    $heading = _AM_PMW_LABEL_TABLE_ONE_DEL;
    print "
    <form action='database_table.php' method='POST' enctype='application/x-www-form-urlencoded'>\n
    <table border='1' cellpadding='0' cellspacing='0' width='100%'>\n
        <tr><th>${heading}</th></tr>\n
    ...
    print "<tr>
                ...
                <td class='even'>\n
                    ...
                    <input type='submit' value='" . _AM_PMW_BUT_DELETE . "'>\n
                </td></tr>\n
            </table>\n
        </td></tr>\n
    </table>\n
...
function PackMasterWeb_get_table_one_result($table_one_key = null)
{
    global $xoopsDB;
    $bci = Utility::getTableOneField();

    $sql = 'select ' . Utility::toString($bci) . ' from ' . $xoopsDB->prefix('PackMasterWeb_table_one');
    ...
    if (null !== $table_one_key) {
        ...
        if (!is_numeric($table_one_key)) {
            $error = [
                'msg'  => _AM_PMW_ERROR_TABLE_ONE_BAD_KEY,
                'data' => $sql,
            ];
...
// returns nothing
function PackMasterWeb_table_one_main_select(): void
{
    // POST, except when debugging
    $method = 'POST';
    global $xoopsDB;
    [$result, $error] = PackMasterWeb_get_table_one_result();
    if ($error) {
        printf(_AM_PMW_FMT_ERROR, $error['msg'], $error['data']);
    ...
    print '<th>' . _AM_PMW_LABEL_TABLE_ONE_EDIT2 . "</th>\n";
    print "</tr>\n";
    $are_there_any = false;
    while (false !== ($values = $xoopsDB->fetchArray($result))) {
        ...
        print "<input type='submit' value='" . _AM_PMW_BUT_EDIT . "'>\n";
        print "</form>\n";
        print "</td>\n";
        print "</tr>\n";
    }
    if (!$are_there_any) {
        print "<tr><td class = 'even' colspan = '" . (count($field_list) + 1) . "'>" . _AM_PMW_ERROR_TABLE_ONE_NONE . '</td></tr>';
    }
    print "</table>\n";
    print "</td></tr>\n";
    print "</table>\n";
...
{
    $p_title = _AM_PMW_TABLE_ONE_TITLE;
    xoops_cp_header();
    print "<h4 style='text-align:left;'>${p_title}</h4>";
    // Utility::horizontalMenuAdmin();
    PackMasterWeb_table_one_add_form();
    PackMasterWeb_table_one_main_select();
    xoops_cp_footer();
    exit();
}

// Edit one row from table_one
function PackMasterWeb_table_one_main_select_post(): void
{
    xoops_cp_header();
    $p_title = _AM_PMW_TABLE_ONE_TITLE;
    print "<h4 style='text-align:left;'>${p_title}</h4>";
    // Utility::horizontalMenuAdmin();
    PackMasterWeb_table_one_edit_form();
    xoops_cp_footer();
    exit();
}

// Delete one row from table_one
function PackMasterWeb_table_one_delete(): void
{
    global $xoopsDB;
    global $param_table_one_key;

    $sql = 'delete from ' . $xoopsDB->prefix('PackMasterWeb_table_one') . ' where table_one_key = ' . "'" . $GLOBALS['xoopsDB']->escape($param_table_one_key) . "'";

    if (!$xoopsDB->query($sql)) {
        $error = $xoopsDB->error();
        xoops_cp_header();
        Utility::showSqlError(_AM_PMW_ERROR_DELETE_FAILED, $error, $sql);
        xoops_cp_footer();
    } else {
        redirect_header('database_table.php', 1, _AM_PMW_OK_DB);
    }
    exit();
}
...
function PackMasterWeb_table_one_update(): void
{
    global $xoopsDB;
    $field_list = Utility::getTableOneField();
    foreach ($field_list as $field => $prompt) {
    ...
    $sql = 'REPLACE INTO  ' . $xoopsDB->prefix('PackMasterWeb_table_one') . ' (' . Utility::toString($field_list) . ') VALUES (';
    ...
    if (!$xoopsDB->query($sql)) {
        $error = $xoopsDB->error();
        xoops_cp_header();
        Utility::showSqlError(_AM_PMW_ERROR_REPLACE_FAILED, $error, $sql);
        xoops_cp_footer();
    } else {
        redirect_header('database_table.php', 1, _AM_PMW_OK_DB);
    }
...
switch ($param_op) {
    case 'main':
        ...
        break;
    case 'select_post':
        PackMasterWeb_table_one_main_select_post();
        break;
    case 'edit_post':
        PackMasterWeb_table_one_update();
        break;
    case 'add_post':
        PackMasterWeb_table_one_add();
        break;
    case 'del_post':
        PackMasterWeb_table_one_delete();
        break;
    default:
        xoops_cp_header();
