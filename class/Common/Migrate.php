...
namespace XoopsModules\Packmasterweb\Common;
...
class Migrate extends \Xmf\Database\Migrate
{
    ...
    /**
     * Migrate constructor.
     * @param \XoopsModules\Packmasterweb\Common\Configurator|null $configurator
     */
            ...
            if (false !== mb_strpos($attributes, ' int(')) {
                if (false === mb_strpos($attributes, 'unsigned')) {
                    ...
                    $this->tableHandler->update(
                        $tableName,
                        [
                            $columnName => "4294967296 + ${columnName}",
                        ],
                        "WHERE ${columnName} < 0",
                        false
                    );
                ...
                $this->tableHandler->update(
                    $tableName,
                    [
                        $columnName => "INET_NTOA(${columnName})",
                    ],
                    '',
                    false
                );
